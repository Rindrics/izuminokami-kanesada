...
3. **人物の特定**: 人物の発言マップを作成する際は、以下の情報を組み合わせて判定
   - `mentioned` フィールド（トップレベル）: このコンテンツ内で言及されている人物のリスト
...
segments:
  ...
  - text: 克己復禮為仁
    speaker: kongzi  # 孔子が直接話している
mentioned: [yanyuan]  # トップレベル: このコンテンツ内で顔淵が言及されている
```

この場合:
- 音声生成: 最初の2セグメントはナレーター（女性）、最後は孔子（男性）
- 発言マップ作成: 「顏淵問仁」セグメントから「顔淵が質問している」ことを判定可能（`mentioned` に `yanyuan` が含まれ、動詞「問」から判定）
...
2. `speaker: null` かつ `mentioned`（トップレベル）に人物が含まれる場合:
